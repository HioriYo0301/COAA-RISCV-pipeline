...
module core import common::*;(
	...
	core_decode decode(.instr(instr_id), .decode_out(decode_id));
	assign rd_id=decode_id.rd; assign rs1_id=decode_id.rs1; assign rs2_id=decode_id.rs2;
	assign funct3_id=decode_id.funct3; assign funct7_id=decode_id.funct7; assign imm_id=decode_id.imm;
	assign alu_op_id=decode_id.alu_op; assign alu_src_id=decode_id.alu_src; assign use_pc_id=decode_id.use_pc;
	assign mem_read_id=decode_id.mem_read; assign mem_write_id=decode_id.mem_write; assign reg_write_id=decode_id.reg_write;
	assign is_branch_id=decode_id.is_branch; assign is_jump_id=decode_id.is_jump; assign is_jalr_id=decode_id.is_jalr; assign wb_sel_id=decode_id.wb_sel;

	core_regfile regfile(.clk(clk), .reset(reset), .wen(reg_write_wb_fire), .waddr(rd_wb), .wdata(wb_data), .raddr1(rs1_id), .raddr2(rs2_id), .rdata1(rs1_data_id_r), .rdata2(rs2_data_id_r), .regs_dbg(rf_dbg));
	...
	always_ff @(posedge clk) begin
		if (reset || redirect_fire_ex) begin
			pc_ex <= 64'b0; instr_ex <= 32'b0; inst_valid_ex <= 1'b0; rs1_data_ex <= 64'b0; rs2_data_ex <= 64'b0;
			rd_ex <= 5'b0; rs1_ex <= 5'b0; rs2_ex <= 5'b0; imm_ex <= 64'b0; funct3_ex <= 3'b0; funct7_ex <= 7'b0;
			alu_op_ex <= ALU_ADD; alu_src_ex <= 1'b0; use_pc_ex <= 1'b0; mem_read_ex <= 1'b0; mem_write_ex <= 1'b0;
			reg_write_ex <= 1'b0; is_branch_ex <= 1'b0; is_jump_ex <= 1'b0; is_jalr_ex <= 1'b0; wb_sel_ex <= WB_ALU;
			...
			pc_ex <= pc_id; instr_ex <= instr_id; inst_valid_ex <= inst_valid_id; rs1_data_ex <= rs1_data_id_r; rs2_data_ex <= rs2_data_id_r;
			rd_ex <= rd_id; rs1_ex <= rs1_id; rs2_ex <= rs2_id; imm_ex <= imm_id; funct3_ex <= funct3_id; funct7_ex <= funct7_id;
			alu_op_ex <= alu_op_id; alu_src_ex <= alu_src_id; use_pc_ex <= use_pc_id; mem_read_ex <= mem_read_id; mem_write_ex <= mem_write_id;
			reg_write_ex <= reg_write_id; is_branch_ex <= is_branch_id; is_jump_ex <= is_jump_id; is_jalr_ex <= is_jalr_id; wb_sel_ex <= wb_sel_id;
		end
	end

	logic load_use_hazard;
	core_hazard_unit hazard_unit(.mem_read_ex(mem_read_ex), .rd_ex(rd_ex), .rs1_id(rs1_id), .rs2_id(rs2_id), .load_use_hazard(load_use_hazard));
	...
	core_forwarding_unit forwarding_unit(
		.rs1_ex(rs1_ex), .rs2_ex(rs2_ex), .rs1_data_ex(rs1_data_ex), .rs2_data_ex(rs2_data_ex),
		.reg_write_mem(reg_write_mem), .rd_mem(rd_mem), .forward_data_mem(forward_data_mem),
		.reg_write_wb(reg_write_wb), .rd_wb(rd_wb), .wb_data(wb_data),
		.op_a_forwarded(rs1_forwarded_ex), .rs2_forwarded(rs2_forwarded_ex)
	);

	assign alu_in_a_ex = use_pc_ex ? pc_ex : rs1_forwarded_ex;
	assign alu_in_b_ex = alu_src_ex ? imm_ex : rs2_forwarded_ex;
	core_alu alu(.alu_op(alu_op_ex), .op_a(alu_in_a_ex), .op_b(alu_in_b_ex), .result(alu_result_ex));
	...
	always_ff @(posedge clk) begin
		if (reset) begin
			pc_mem <= 64'b0; instr_mem <= 32'b0; inst_valid_mem <= 1'b0; alu_result_mem <= 64'b0; rs2_data_mem <= 64'b0;
			rd_mem <= 5'b0; funct3_mem <= 3'b0; mem_read_mem <= 1'b0; mem_write_mem <= 1'b0; reg_write_mem <= 1'b0; wb_sel_mem <= WB_ALU;
		end else if (!fetch_wait && !mem_wait) begin
			pc_mem <= pc_ex; instr_mem <= instr_ex; inst_valid_mem <= inst_valid_ex; alu_result_mem <= alu_result_ex; rs2_data_mem <= rs2_forwarded_ex;
			rd_mem <= rd_ex; funct3_mem <= funct3_ex; mem_read_mem <= mem_read_ex; mem_write_mem <= mem_write_ex; reg_write_mem <= reg_write_ex; wb_sel_mem <= wb_sel_ex;
		end
	end
	...
	always_ff @(posedge clk) begin
		if (reset) begin
			pc_wb <= 64'b0; instr_wb <= 32'b0; inst_valid_wb <= 1'b0; alu_result_wb <= 64'b0; mem_data_wb <= 64'b0; rd_wb <= 5'b0;
			funct3_wb <= 3'b0; mem_read_wb <= 1'b0; mem_write_wb <= 1'b0; reg_write_wb <= 1'b0; wb_sel_wb <= WB_ALU;
		end else if (!fetch_wait && !mem_wait) begin
			pc_wb <= pc_mem; instr_wb <= instr_mem; inst_valid_wb <= inst_valid_mem; alu_result_wb <= alu_result_mem; mem_data_wb <= load_data_mem;
			rd_wb <= rd_mem; funct3_wb <= funct3_mem; mem_read_wb <= mem_read_mem; mem_write_wb <= mem_write_mem; reg_write_wb <= reg_write_mem; wb_sel_wb <= wb_sel_mem;
		end
	end
	...
	always_ff @(posedge clk) begin
		if (reset) wb_fired <= 1'b0;
		else if (!fetch_wait && !mem_wait) wb_fired <= 1'b0;
		else if (commit_valid_wb) wb_fired <= 1'b1;
	end

	always_ff @(posedge clk) begin
		if (reset) begin
			cycle_cnt <= 64'b0; instr_cnt <= 64'b0;
		end else begin
			cycle_cnt <= cycle_cnt + 64'd1;
			if (commit_valid_wb && !is_trap_wb) instr_cnt <= instr_cnt + 64'd1;
		end
	end
...
`ifdef VERILATOR
	DifftestInstrCommit DifftestInstrCommit(
		.clock(clk), .coreid(8'b0), .index(8'b0), .valid(commit_valid_wb), .pc(pc_wb), .instr(instr_wb),
		.skip(difftest_skip_wb), .isRVC(1'b0), .scFailed(1'b0), .wen(reg_write_wb_fire), .wdest({3'b0, rd_wb}), .wdata(wb_data)
	);

	DifftestArchIntRegState DifftestArchIntRegState (
		.clock(clk), .coreid(8'b0),
		.gpr_0(gpr_dt[0]), .gpr_1(gpr_dt[1]), .gpr_2(gpr_dt[2]), .gpr_3(gpr_dt[3]), .gpr_4(gpr_dt[4]), .gpr_5(gpr_dt[5]), .gpr_6(gpr_dt[6]), .gpr_7(gpr_dt[7]),
		.gpr_8(gpr_dt[8]), .gpr_9(gpr_dt[9]), .gpr_10(gpr_dt[10]), .gpr_11(gpr_dt[11]), .gpr_12(gpr_dt[12]), .gpr_13(gpr_dt[13]), .gpr_14(gpr_dt[14]), .gpr_15(gpr_dt[15]),
		.gpr_16(gpr_dt[16]), .gpr_17(gpr_dt[17]), .gpr_18(gpr_dt[18]), .gpr_19(gpr_dt[19]), .gpr_20(gpr_dt[20]), .gpr_21(gpr_dt[21]), .gpr_22(gpr_dt[22]), .gpr_23(gpr_dt[23]),
		.gpr_24(gpr_dt[24]), .gpr_25(gpr_dt[25]), .gpr_26(gpr_dt[26]), .gpr_27(gpr_dt[27]), .gpr_28(gpr_dt[28]), .gpr_29(gpr_dt[29]), .gpr_30(gpr_dt[30]), .gpr_31(gpr_dt[31])
	);

    DifftestTrapEvent DifftestTrapEvent(
		.clock(clk), .coreid(0), .valid(trap_valid_wb), .code(trap_code_wb[2:0]), .pc(pc_wb), .cycleCnt(cycle_cnt), .instrCnt(instr_cnt)
	);

	DifftestCSRState DifftestCSRState(
		.clock(clk), .coreid(0), .priviledgeMode(3), .mstatus(0), .sstatus(0), .mepc(0), .sepc(0), .mtval(0), .stval(0), .mtvec(0), .stvec(0),
		.mcause(0), .scause(0), .satp(0), .mip(0), .mie(0), .mscratch(0), .sscratch(0), .mideleg(0), .medeleg(0)
	);
`endif
endmodule
`endif
